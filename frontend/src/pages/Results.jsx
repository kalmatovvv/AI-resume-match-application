...
export function ResultsPage({ results, loading }) {
  ...
  const industries = React.useMemo(() => {
    if (!results) return [];
    const set = new Set(results.map((r) => r.industry).filter(Boolean));
    return Array.from(set).sort();
  }, [results]);

  const filteredResults = React.useMemo(() => {
    if (!results) return [];
    if (!selectedIndustry) return results;
    return results.filter((r) => r.industry === selectedIndustry);
  }, [results, selectedIndustry]);
        ...
        {industries.length > 0 && (
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`px-3 py-1 text-xs font-medium rounded-full transition border ${selectedIndustry === null
            ...
            {industries.map((industry) => (
              <button
                ...
                className={`px-3 py-1 text-xs font-medium rounded-full transition border ${selectedIndustry === industry
                  ? 'bg-emerald-500 border-emerald-500 text-slate-950 shadow-sm'
                  : 'bg-slate-900 border-slate-800 text-slate-400 hover:border-slate-700 hover:text-slate-200'
                  }`}
              >
